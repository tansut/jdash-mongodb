"use strict";
var helper_1 = require("./helper");
var should = require('should');
function default_1() {
    var dashboardCount = 0;
    describe('dashboard', function () {
        it('should create a dashboard', function () {
            var provider = helper_1.default.provider;
            var dashboardCreateModel = {
                title: 'Foo title',
                description: 'eewrew',
                id: "",
                user: helper_1.default.testUser
            };
            return provider.createDashboard(dashboardCreateModel).then(function (result) {
                dashboardCount++;
            });
        });
        it('should populate 100 dashboards for user', function () {
            var provider = helper_1.default.provider;
            var promises = [];
            for (var i = 0; i < 100; i++) {
                var dashboardCreateModel = {
                    title: 'Foo title' + i,
                    description: 'eewrew',
                    id: "",
                    user: helper_1.default.testUser
                };
                promises.push(provider.createDashboard(dashboardCreateModel));
                dashboardCount++;
            }
            return promises;
        });
        it('should create and gets the dashboard', function () {
            var provider = helper_1.default.provider;
            var newDashboard = {
                title: 'Foo title',
                description: 'eewrew',
                id: "",
                user: helper_1.default.testUser
            };
            return provider.createDashboard(newDashboard).then(function (result) {
                dashboardCount++;
                return provider.getDashboard(result.id);
            });
        });
        it('should get users dashboards all', function () {
            var provider = helper_1.default.provider;
            return provider.getDashboardsOfUser(helper_1.default.testUser).then(function (dashes) {
                should.equal(dashes.data.length, 102);
                should.equal(dashes.hasMore, false);
            });
        });
        it('should get users first 5 dashboards', function () {
            var provider = helper_1.default.provider;
            return provider.getDashboardsOfUser(helper_1.default.testUser, { limit: 5, startFrom: 0 }).then(function (dashes) {
                should.equal(dashes.data.length, 5);
                should.equal(dashes.hasMore, true);
            });
        });
        it('should get users first 15 dashboards', function () {
            var provider = helper_1.default.provider;
            return provider.getDashboardsOfUser(helper_1.default.testUser, { limit: 15, startFrom: 0 }).then(function (dashes) {
                should.equal(dashes.data.length, 15);
                should.equal(dashes.hasMore, true);
            });
        });
        it('should get users first 150 dashboard', function () {
            var provider = helper_1.default.provider;
            return provider.getDashboardsOfUser(helper_1.default.testUser, { limit: 150, startFrom: 0 }).then(function (dashes) {
                should.equal(dashes.data.length, dashboardCount);
                should.equal(dashes.hasMore, false);
            });
        });
        it('should get users first 20 dashboard from start index 90', function () {
            var provider = helper_1.default.provider;
            return provider.getDashboardsOfUser(helper_1.default.testUser, { limit: 20, startFrom: 90 }).then(function (dashes) {
                should.equal(dashes.data.length, 12);
                should.equal(dashes.hasMore, false);
            });
        });
    });
}
Object.defineProperty(exports, "__esModule", { value: true });
exports.default = default_1;
